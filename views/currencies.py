...
# Core FX universe surfaced in the sidebar and default selections.
...
# Orange-to-grey palette aligned with the rest of the dashboard.
...
# Shared heatmap gradient for correlation views.
HEATMAP_COLOR_SCALE = [
    [0.0, "#4A4A4A"],
    [0.5, "#FFFFFF"],
    [1.0, "#FF962F"],
...
# Local cache for Yahoo FX extracts; safe to delete to force refresh.
DATA_DIR = Path("data")
DATA_DIR.mkdir(exist_ok=True)
DATA_PATH = DATA_DIR / "fx_prices.csv"


def _refresh_cached_dataset() -> pd.DataFrame:
    """Download the full FX universe and persist it to disk."""
    dataset = download_data(
        tuple(CURRENCY_TICKERS.keys()),
        DEFAULT_START_DATE,
        DEFAULT_END_DATE,
        use_live=True,
    )
...
def _load_full_dataset(use_live: bool) -> pd.DataFrame:
    """Load cached FX data, refreshing according to user preference."""
    if DATA_PATH.exists():
        if use_live:
...
def load_currency_prices(symbols: Tuple[str, ...], start: date, end: date, use_live: bool) -> pd.DataFrame:
    """Retrieve FX prices for the selected universe and date range."""
    if not symbols:
        return pd.DataFrame()
...
def build_color_map(labels: Iterable[str]) -> Dict[str, str]:
    """Construct a color map keyed by currency pair tickers."""
    fallback_colors = px.colors.qualitative.Set3
...
def render_top_gainers(metrics: pd.DataFrame, closing_prices: pd.Series) -> None:
    """Display top-performing currency pairs in card format."""
    ...
    def build_entry(label: str, value: float) -> str:
        ...
        price_text = f"{last_price:,.4f}" if pd.notna(last_price) else "N/A"
...
def render_performance_chart(
    ...
    fig.update_layout(
        title=(
            "Global FX Dashboard<br>"
            "<span style=\"font-size:0.85em;font-weight:400;\">"
            "Normalized performance view to compare momentum across the selected currency pairs."
            "</span>"
        ),
...
def render_volatility_drawdown_tab(
    ...
    """Render volatility and drawdown analytics."""
    if rolling_vol.empty:
        ...
        vol_fig.update_layout(
            ...
            title=(
                ...
                "<span style=\"font-size:0.85em;font-weight:400;\">"
                "Annualized σ derived from a 30-day rolling window of FX daily returns."
                "</span>"
            ),
            xaxis_title="Date",
    ...
    dd_fig.update_layout(
        ...
        title=(
            ...
            "<span style=\"font-size:0.85em;font-weight:400;\">"
            "Illustrates peak-to-trough drawdowns so you can gauge FX weakness depth and recovery."
            "</span>"
        ),
        xaxis_title="Date",
...
def render_correlation_tab(returns: pd.DataFrame) -> None:
    """Render correlation heatmap of daily returns."""
    if returns.empty or returns.shape[1] < 2:
        st.info("Correlation analysis requires at least two FX instruments with history.")
        return
    ...
    heatmap.update_layout(
        ...
        title=(
            ...
            "<span style=\"font-size:0.85em;font-weight:400;\">"
            "Pairwise correlations of daily FX returns to uncover shared macro drivers."
            "</span>"
        ),
        xaxis_title="",
        yaxis_title="",
...
def render() -> None:
    """Entry point for the FX tab."""
    try:
        st.set_page_config(
            page_title="Currencies | Cross-Asset Market Monitor",
    ...
    st.caption("Interactive analytics covering major global currencies and FX indices.")

    default_start = max(DEFAULT_START_DATE, DEFAULT_END_DATE - timedelta(days=365))

    with st.sidebar:
        options = list(CURRENCY_TICKERS.keys())
    ...
    if not selected_symbols:
        st.info("Select at least one FX instrument from the sidebar to begin the analysis.")
        return

    prices = load_currency_prices(tuple(selected_symbols), start_date, end_date, use_live=True)
    if prices.empty:
    ...
    st.dataframe(export_frame.style.format("{:.4f}"))

    csv_bytes = export_frame.to_csv(index_label="Date").encode("utf-8")
    st.download_button(
        "Download CSV",
        data=csv_bytes,
        file_name=f"currencies_raw_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.csv",
        mime="text/csv",
        key="download_currencies_raw",
    )
